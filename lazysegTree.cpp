...
// this class is used for lazy propogation when the range updates are given such that  update l to r with val.
class ST{
private: 
    vector<int> sg, lazy;
public:
    ...
    void update(int index, int low, int high, int l, int r, int val){
        ...
        if(lazy[index] != 0){
            sg[index] += (high - low + 1) * lazy[index];
        ...
        // it has steps 
        // first update then lazy propogate
        if(l <= low and high <= r){
            sg[index] += (high - low + 1) * val;
    ...
    int query(int index, int low, int high, int l, int r){
        ...
        if(lazy[index] != 0){
            ...
            // if our lazy node has children then propogate to them
            if(low != high){
                lazy[2*index + 1] += lazy[index];
                lazy[2*index + 2] += lazy[index];
            }
...
//this class will do the min range query and we also have to update the range to val 

// there will be some changes then the above class
class STMin{ 
private: 
    vector<int> sg, lazy;
public:
    ...
    void update(int index, int low, int high, int l, int r, int val){
        ...
        //update the previous remaining updates and propgate downwards
        if(lazy[index] != 0){
            sg[index] += lazy[index]; // this step is different cause now the sg[index] will hold the value which is min and we have to update the val 
            // so min + val will be now the new min at the sg[index]
...
void solve() {
    ...
    cout << st.query(0, 0, n - 1, 1, 3) << endl; //15
    st.update(0, 0, n - 1, 1, 5, 10);
    cout << st.query(0, 0, n - 1, 1, 3);//45
}

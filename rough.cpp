...
/*
	state dp[i][j] smallest string starting from 00 to ij

	transition
		if(grid[i + 1][j] != grid[i][j + 1]) whoever is smaller we will go that side

		else min of both side
...
void solve() {
	...
	for(int j = 1; j < n; ++j) dp[j][0] = dp[j - 1][0] + grid[j][0];
